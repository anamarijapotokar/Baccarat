...
\newpage
\noindent The Banker drawing rules depend on whether the Player draws a third card:

\begin{itemize}
    \item If the Player \textbf{does not} draw a third card (i.e.\ the Player stands on a total of 6 or 7), then the Banker draws one card when their first two cards total \textbf{0--5}, and stands when their total is \textbf{6 or 7}.
    \item If the Player \textbf{does} draw a third card, then the Banker follows the drawing rules described in the table below, which depend on both the Banker's total and the value of the Player's third card.
\end{itemize}
\begin{center}
...
\noindent If the outcome is a tie, we get our pay in back.
